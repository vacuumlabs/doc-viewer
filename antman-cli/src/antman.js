...
import {upload as _upload, link as _link, home as _home} from './upload.js'
...
const usageHome = 'antman home -k api_key host folder'
...
function home(args) {
  ...
  if (!(folder && host && args.k)) {
    console.log(`usage: ${usageHome}`)
    process.exit(0)
  }

  run(function* () {
    yield run(_home, args.k, host, folder)
  })
}
...
const action = {upload, home, migrate, init}[command]
